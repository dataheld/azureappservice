...
ENV RSPM_SNAPSHOT_URL=https://packagemanager.rstudio.com/all/__linux__/focal/$RSPM_SNAPSHOT_DATE+$RSPM_SNAPSHOT_QUERY
...
FROM base AS runner
ENV R_LIBS=$R_LIBS_RUNTIME
# TODO remove redundant installation of dependencies
# this is blocked by an issue in pak
COPY --link --from=dep_installer $R_LIBS_RUNTIME $R_LIBS_RUNTIME
COPY --link --from=installer $R_LIBS_RUNTIME $R_LIBS_RUNTIME
COPY --link --from=roxygeniser $SOURCE_MOUNT_PATH/docs $SOURCE_MOUNT_PATH/docs
WORKDIR $SOURCE_MOUNT_PATH
ENV R_LIBS=$R_LIBS_RUNTIME
# this allows below function to safely default to localhost when R_HOST is unset
ENV R_HOST="0.0.0.0"
# port for pkgdown, passed to servr in below serve_all
EXPOSE 3840
...
# set inside GitHub Actions see
# https://docs.github.com/en/actions/learn-github-actions/environment-variables
# should be last to avoid invalidating earlier layers
ARG GITHUB_SHA
